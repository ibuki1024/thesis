...
In the case where both the state and action space are continuous, the problem is that it is very expensive to obtain $\argmax_aQ^{\pi}(s,a)$. Thus, up to this point, the policy $\pi$ has been determined by the  $Q$ function, but this approach cannot be taken when both spaces are continuous. 
Therefore, the policy function is often parameterized as $\pi_{\theta}$ and the parameter $\theta$ is updated by gradient method. 

\subsection{Deterministic Policy Gradient Method}
Silver et al.\cite{DPG} find the gradient for the evaluation function $J(\pi_{\theta})$, even if the policy $\pi(s)$ is defined as deterministic policy. This gradient is known as deteministic policy gradient(DPG), and it is calculate as following theorem.
...
\end{equation}
Therefore, the accuracy of the approximation of the policy gradient largely depends on the accuracy of the critic approximation and the distribution of the mini-batch $E$.
...
\end{equation}
at each point sampled in Figure \ref{Q_approximation} is 0.03. Thus, we can consider that the critic learned by Algorithm \ref{alg1} is a good approximation of $V^{\pi}$. The dependency of computation complexity for dimension of state $s$ should be studied in the future.
\if0
Depending on the dimensions of the state variables, the number of epochs required for TD learning is shown in Table \ref{epoch}. 
\begin{table}[htb]
  \begin{center}
...
\end{figure}\\
The activation function ``original" shown in Figure \ref{NN} is defined as $9.99 \times \textrm{sigmoid} + 0.01$ to meet upper and lower limits of interval described in section 5.2 and 5.3.\par
