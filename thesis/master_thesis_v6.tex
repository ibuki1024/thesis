...
where
\begin{equation}
	V^{\pi}(s) = \int_{0}^{\infty} e^{-\alpha t}\expect_{w}[s(t)^{\top}Es(t)+u(t)^{\top}Fu(t)+\beta \delta(t)C(t)|s(0)=s]\textrm{d}t \label{such_that}
\end{equation}
...
\begin{align}
	V^{\pi}(s) &= \sum_{i=0}^{\infty}\int_{t_i}^{t_{i+1}} e^{-\alpha t}\expect_{w}[s(t)^{\top}Es(t)+u_i^{\top}Fu_i+\beta \delta(t)C(t)|s(0)=s]\textrm{d}t \nonumber \\
