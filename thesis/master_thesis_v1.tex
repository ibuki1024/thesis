...
\end{equation}
should be met. \par
Now, during the reinforcement learning process, the policy keeps changing at each step, so $Q^{\pi}(s,a)$, which is the approximation target of critic, also keeps changing. In addition, the empirical data used for critic's learning includes data of experience with policies different from the one to be evaluated. Thus, there is no guarantee that $Q^{\pi_{\textrm{RL}}}(s,a)$ is approximated correctly at each step. Therefore, we learned $Q^{\pi_{\textrm{RL}}}(s,a)$ by supervised learning of critic using only the data generated by interaction with a fixed policy.  Let $Q_{\textrm{ap}}^{\pi_{\textrm{RL}}}(s,a)$ denotes this approximated function. \par
If $\pi_{\textrm{RL}}$ is the optimal policy, $\nabla_aQ_{\textrm{ap}}^{\pi_{\textrm{RL}}}(s,a)|_{\pi_{\textrm{RL}}}$ should be $\bm{0}$. However, when this is calculated in practice, we have
\begin{equation}
	\nabla_aQ_{\textrm{ap}}^{\pi_{\textrm{RL}}}(s,a) = \begin{bmatrix}0.03, 0.07\end{bmatrix}.
\end{equation}
Therefore, we find $\pi_{\textrm{RL}}$ is not the optimal policy.
...
\end{equation}
となるべきである. \par
方策$\pi_{\textrm{RL}}$が式\eqref{optimality}を満たしているかを調査するために, $Q^{\pi_{\textrm{RL}}}(s,a)$を求めたい.そこで, 価値関数$V^{\pi_{\textrm{RL}}}(s) = \sum_{i=0}^{\infty}$の近似値をシミュレーションによって以下のように求め, 
\begin{equation}
	V^{\pi_{\textrm{RL}}}_{\textrm{ap}}(s) = \sum_{i=0}^{N}\gamma^i r(s_i,\pi_{\textrm{RL}}(s_i))
\end{equation}
$V^{\pi_{\textrm{RL}}}_{\textrm{ap}}$を用いて$Q^{\pi_{\textrm{RL}}}$の近似値を以下のようにして求める.
\begin{equation}
	Q^{\pi_{\textrm{RL}}}_{\textrm{apd}}(s,a) = r(s,a) + \gamma V^{\pi_{\textrm{RL}}}_{\textrm{ap}}(s^{\prime})
\end{equation}
$\gamma < 1$なので, シミュレーションのステップ数$N$を大きく取っておけば, $Q^{\pi_{\textrm{RL}}}_{\textrm{apd}}$はいい近似となる. ただし, 方策勾配の計算には$\nabla_{a}Q^{\pi_{\textrm{RL}}}(s, a)$を計算する必要があるが, $Q^{\pi_{\textrm{RL}}}_{\textrm{apd}}$は$(s,a)$における値を計算することしかできない. そこで, 適当な関数近似器を$Q^{\pi_{\textrm{RL}}}_{\textrm{apd}}(s,a)$を教師データとして教師あり学習することで関数$Q^{\pi_{\textrm{RL}}}$を近似する. この関数を$Q^{\pi_{\textrm{RL}}}_{\textrm{ap}}$とする.\par
さて, $Q^{\pi_{\textrm{RL}}}_{\textrm{ap}}$を用いて方策勾配を計算すると
\begin{equation}
	\expect_{s\sim\rho^{\pi_{\textrm{RL}}}}[
	\nabla_{\theta}\pi_{\textrm{RL}}(s)\nabla_{a}Q^{\pi_{\textrm{RL}}}(s, a)|_{a=\pi_{\textrm{RL}}(s)}] \neq \bm{0} 
\end{equation}
となり, 方策$\pi_{\textrm{RL}}$は最適方策ではないことがわかった.
